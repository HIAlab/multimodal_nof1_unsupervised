...
def return_gls_pvalues_for_all_ids(nlme, scrt_dfs, IDs, treatment_columns):
    base = rpackages.importr("base")
    fmla_intervention = Formula("Value ~ Intervention")
    gls_fits = {}
...
def calculate_tests_from_embeddings(
    ...
    for rad in radius:
        for scen in scenarios:
            ...
            dataset, _ = return_complete_data_and_pcas_from_paths(
            ...
            split_full_datas = prep_ftest_data(
                data,
            ...
            scrt, nlme = prep_rpy2_packages()
            scrt_dfs = prep_rpy_dfs(
                data,
                IDs,
            ...
            if "scrt" in tests_to_run:
                try:
                    scrt_pvalues = return_scrt_pvalues(
                        scrt,
                        scrt_dfs,
                        IDs,
                        design="ABAB",
            ...
            if "mlx" in tests_to_run:
                try:
                    mlxtend_permutation_results = run_mlxtend_permutation_tests(
                        data,
                        IDs,
                        treatment_column,
                        target_column,
            ...
            all_vals = get_all_results(IDs, treatment_column, resultslist)
            if sim:
                all_alpha_vals = prep_alpha_vals(all_vals)
